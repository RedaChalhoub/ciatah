function [fileInfo] = getFileInfo(fileStr, varargin)
	...
	options.assayList = {'EXT|Ext|RST|Rst|PostAbs|Postabs|postAbs|postabs|PA|SA|REN|REINST|CuedRST|CuedRst|Baseline|baseline'};
	%options.assayList = {...
	...
	options.subjectRegexp = '(D1\-|D2\-|m|M|f|F|Mouse|mouse|Mouse_|mouse_)\d+';
	options.originalStr = {'PAV-PROBE','SAL','SULP','SHC','REINST','EXT','PA', 'CuedRST', 'CuedRst','Rst', 'baseline', 'PostAbs', 'Postabs', 'postAbs','postabs'};
	options.replaceStr = {'PAVQ','SCH','SUL','SCH','RST','EXT','PA','RST','RST','RST', 'Baseline','PA', 'PA', 'PA','PA'};
	options.dateRegexp = '(\d{8}|\d{6}|\d+_\d+_\d+|\d{4}\-\d{2}\-\d{2})';
	% regular expression for different layers in 2P or related data
	options.planeRegexp = 'efocus\d+';
    ...
    % Correct for PA sessions
    if isempty(fileInfo.assay)
        idx1=strfind(fileStr,'\');
        foldername=fileStr(idx1(end)+1:end);
        fileInfo.assay = regexp(foldername,assayListOriginal, 'match');
    end
